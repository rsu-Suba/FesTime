...
export default function SpotsDebugPage() {
  ...
  const downloadAllAsZip = async () => {
    ...
    setProgress(0);
    const zip = new JSZip();
    const hide = message.loading("Generating all QR codes", 0);
    ...
    try {
      ...
      for (let i = 0; i < spots.length; i++) {
        ...
        if (element) {
          ...
          zip.file(`spot_qr_${spot.id}.jpg`, base64Data, { base64: true });
        }
      ...
      const link = document.createElement("a");
      link.download = `FesTime_all_spots_qr_highres.zip`;
      link.href = URL.createObjectURL(content);
      link.click();

      message.success("Downloaded all QR codes.");
    } catch (err) {
      console.error(err);
      message.error("Failed to generate all QR codes");
    } finally {
      if (bulkRef.current) {
        ...
        >
          ...
          <Space orientation="vertical" size="large" style={{ width: "100%" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              <Button type="primary" danger ghost onClick={downloadAllAsZip} disabled={isExporting} block>
                All as ZIP
              </Button>
            </div>
      ...
      <div ref={bulkRef} style={{ position: "absolute", top: "-9999px", left: "-9999px", display: "none" }}>
        {spots.map((spot) => (
          <div
            key={spot.id}
            id={`bulk-qr-${spot.id}`}
            style={{ background: "#fff", padding: "80px", width: "1200px", textAlign: "center" }}
          >
            <h1 style={{ fontSize: "72px", marginBottom: "40px" }}>{spot.location}</h1>
            <div style={{ display: "inline-block", border: "2px solid #000", padding: "20px" }}>
              <CustomQRCode value={`${baseUrl}?spot=${spot.id}`} size={800} />
            </div>
            <div style={{ marginTop: "40px", fontSize: "32px", fontWeight: "bold" }}>ID: {spot.id}</div>
          </div>
        ))}
      </div>
